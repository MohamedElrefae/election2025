#!/usr/bin/env python3
"""
Egyptian Election Data - Proper Location Re-extraction (Fixed)
Re-extracts location data with proper column separation and type handling
"""
...
def re_extract_locations():
    ...
    print("=" * 60)
    print("🔄 Egyptian Election Data - Proper Location Re-extraction (Fixed)")
    print("=" * 60)
    ...
    for i, location in enumerate(locations_data):
        try:
            # Extract basic info
            location_id = safe_int(location.get('location_id', i + 1))
            raw_name = str(location.get('location_name', '')).strip()
            raw_address = str(location.get('location_address', '')).strip()
            total_voters = safe_int(location.get('total_voters', 0))
            ...
            if 'location_number' in location and location['location_number']:
                location_number = safe_int(location['location_number'])
            ...
            if not location_number:
                ...
                for field in ['main_committee_id', 'committee_id', 'page_number']:
                    if field in location and location[field]:
                        location_number = safe_int(location[field])
            ...
            # Standard fields
            governorate = str(location.get('governorate', 'كفر الشيخ'))
            district = str(location.get('district', 'مطوبس'))
            ...
            processed_location = {
                ...
                'district': district,
                'main_committee_id': safe_int(location.get('main_committee_id')),
                'police_department': safe_int(location.get('police_department')),
    ...
    for _, row in df.head(15).iterrows():
        loc_num = safe_int(row['location_number'])
        loc_name = str(row['location_name'])[:60]
        voters = safe_int(row['total_voters'])
        print(f"   {loc_num:3d}: {loc_name:<60} ({voters} voters)")
...
def show_extraction_summary():
    ...
    if os.path.exists(output_file):
        ...
        print(f"\n📋 All location names found ({len(unique_names)} total):")
